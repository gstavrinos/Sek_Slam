...
void merge_stream2(){
	double startX = gl_image_2.width/2;
	double startY = gl_image_2.height;
        float min_angle = /*RadiansToDegrees(*/gl_scan.angle_min;//save the minimum angle in degrees
        float max_angle = /*RadiansToDegrees(*/gl_scan.angle_max;//save the maximum angle in degrees
        float inc = /*RadiansToDegrees(*/gl_scan.angle_increment;               //save the step between angles
        float min_range = gl_scan.range_min;            //save both the min and max values because values
	...
	float max_range = 1;// gl_scan.range_max;               //out of these limits need to be discarded.
        ...
        try
        {
                cv_ptr = cv_bridge::toCvCopy(gl_image_2/*, enc::BGR8*/);
                //Mat cv_img_source = cv_ptr->image;
                float currentAngle = min_angle;
                double endX = -100;
                double endY = -100;
                float measure = gl_image_2.width / 2 / max_range;//max range will be in the edges of the screen, and the rest will scale according to our image resolution
                for(unsigned i = 0; i < gl_scan.ranges.size(); i++){
                        if(gl_scan.ranges[i] >= min_range && gl_scan.ranges[i] <= 1.0){//draw only points within the range limits
                                //we move the points from 1st and 4th quadrants, to 2nd and 1st respectively, by adding PI/2 to the degrees
                                endX = startX - currentAngle*gl_image_2.width/2;
		...
		line(cv_ptr->image, cvPoint(0,gl_image_2.height), cvPoint(gl_image_2.width,gl_image_2.height), CV_RGB(0,0,0), 10, 8 ,0);
                circle(cv_ptr->image, cvPoint(startX,startY), 10, CV_RGB(255,0,0), -1);
        }
        catch (cv_bridge::Exception& e)
        {
        ...
        sensor_msgs::ImagePtr msg = cv_ptr->toImageMsg();
        gl_image_2.height = msg->height;
        gl_image_2.width = msg->width;
        gl_image_2.encoding = msg->encoding;
        gl_image_2.step = msg->step;
        gl_image_2.data.clear();
        gl_image_2.data = msg->data;
...
int main(int argc, char** argv)
{
    ...
    ros::Publisher polar_img_pub;
     merged_img_pub = n.advertise<sensor_msgs::Image>("/merged_image", 100);
     polar_img_pub = n.advertise<sensor_msgs::Image>("/merged_image_polar", 100);
    ...
    {   
       ...
       polar_img_pub.publish(gl_image_2);
           SCAN_RECEIVED = 0;
           IMAGE_RECEIVED = 0;
